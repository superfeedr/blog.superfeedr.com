...
For the transportation, there are very few protocols. XMPP and "PubSubHubbub":http://superfeedr.com/hubbub are the ones Superfeedr chose, because *they're both open*, which means that anyone else can implement them and we should be able to work together in a loosely coupled way (without business agreement). 
...
Below is a very simple example. As you know "Gowalla":http://gowalla.com/ pushes it's content in realtime with "PubSubHubbub":http://hub.gowalla.com/. I built a "small server":http://github.com/julien51/socket-sub that can subscribe to Gowalla's content, and then push that to browsers using websockets. I then built a simple HTML + Javascript page (integrated into this "Jekyll":http://github.com/mojombo/jekyll Blog!) which can consume the data pushed to the socket. It's simple, really. I could also build an app that pushes the content to iPhones or one that pushes to my Desktop app.

<div id="map_canvas" style="display:block; width:500px; height: 400px"></div> 
...
$(document).ready(function() {
    ...
    function openConnection() {
        if(!conn.readyState || conn.readyState > 1 ) {
            conn = new WebSocket( "ws://node.superfeedr.com:27261");
    ...
    if (!window.WebSocket) {
        alert("Looks like your browser doesn't support WebSocket. You won't be able to see the awesome demo down the page.")
    } else {
        startConnection();
    }
});
</script>
<small>Please make sure you use a modern browser in its last versions, like Firefox, Safari or Chrome</small>
